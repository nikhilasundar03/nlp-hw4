...
def eval_epoch(args, model, dev_loader, gt_sql_pth, model_sql_path, gt_record_path, model_record_path):
    ...
    # 1) Teacher-forced cross-entropy loss on the dev set.
    total_loss = 0.0
    total_tokens = 0
    with torch.no_grad():
    ...
    # 2) Generation: run model.generate to produce SQL strings for each dev example.
    predictions = []
    with torch.no_grad():
    ...
    # 3) Save SQL + executed records, then compute metrics.
    ...
    # Compute ground-truth dev records once and cache, so repeated eval is cheap.
    if not os.path.exists(gt_record_path):
        import pickle
        from utils import read_queries, compute_records
        gt_qs = read_queries(gt_sql_pth)
