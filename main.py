...
def create_augmented_dataloader(args, dataset):
    ...
    random_indices = random.sample(range(len(train_dataset)), 5000)
    small_subset = train_dataset.select(random_indices)

    transformed_subset = small_subset.map(custom_transform, load_from_cache_file=False)

    transformed_tokenized = transformed_subset.map(tokenize_function, batched=True, load_from_cache_file=False)
    ...
    combined = torch.utils.data.ConcatDataset([original_tokenized, transformed_tokenized])
